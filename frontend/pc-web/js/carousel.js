;(function($){
	$.fn.carousel = function(param){
		...
		autoPlay = param.autoPlay,//自动播放 true/facarousellse
		timer,//定时器
		index = 1,//索引值
		indexList,//下标列表
		indexClassName = "js_index",//下标高亮类名
		action = true;//开始滚动 true/false
		list.css("width",list.width()+"px");
		var totalWidtn = list.width()*(list.length+2),//轮播框架总宽度
        ...
        $(window).resize(function (){
            console.log(document.body.clientWidth,timer);
            list.css("width",document.body.clientWidth);
            index=1;
